...
import { IndemnityService } from '../../../core/services/indemnity.service'; // ← AJOUTÉ
...
export class ClaimsListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private indemnityService: IndemnityService,  // ← AJOUTÉ
  ...
  /**
   * Génère automatiquement l'indemnité avant de naviguer vers la page de détail.
   */
  // ✅ Correct :
    goToIndemnity(claimId: number, event: Event): void {
  event.stopPropagation();
  this.saving = true;

  this.indemnityService.generateIndemnity(claimId).pipe(
    takeUntil(this.destroy$),
    finalize(() => this.saving = false)
  ).subscribe({
    next: (indemnity) => {
      // ✅ Navigation seulement après succès
      this.router.navigate(['/indemnity', claimId]);
    },
    error: (err) => {
      console.error('Erreur génération indemnité:', err);
      let errorMsg = 'Génération impossible';
      if (err.error?.error) errorMsg = err.error.error;
      else if (err.message) errorMsg = err.message;
      this.showError(`Indemnité : ${errorMsg}`);
    }
  });

    // Désactiver le bouton (optionnel) – on utilise `saving` pour l'affichage
    this.saving = true;
    ...
    this.indemnityService.generateIndemnity(claimId).pipe(
      ...
      next: (indemnity) => {
        this.showSuccess(`Indemnité générée - Montant net : ${indemnity.netAmount} DT`);
        // Navigation vers la page de détail de l'indemnité (ou du sinistre)
        this.router.navigate(['/indemnity', claimId]);
      },
      error: (err) => {
        console.error('Erreur génération indemnité:', err);
        ...
        this.showError(`Indemnité : ${errorMsg}`);
        // Navigation quand même ? Décommentez si vous voulez voir la page existante
        // this.router.navigate(['/indemnity', claimId]);
      }
    });
  }
