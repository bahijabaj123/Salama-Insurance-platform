...
export class SidebarComponent implements OnInit {
  ...
  menuItems: MenuItem[] = [
    ...
    { icon: 'fas fa-file-alt', label: 'Sinistres', route: '/claims', badge: 5 },
    { icon: 'fas fa-map', label: 'Carte des sinistres', route: '/map' },
    { icon: 'fas fa-shield-alt', label: 'Anti-fraude', route: '/fraud', badge: 0 },
    { icon: 'fas fa-chart-bar', label: 'Statistiques', route: '/statistiques' },
    { icon: 'fas fa-tools', label: 'Gérer les garages', route: '/garages' } ,
 { icon: 'fas fa-comment-dots', label: 'Complaints', route: '/complaints', badge: 0 }
  ];
  ...
  loadFraudCount(): void {
    this.claimService.getAllClaims().subscribe({
      next: (claims: Claim[]) => {
        ...
        const fraudMenu = this.menuItems.find(m => m.route === '/fraud');
        if (fraudMenu) {
