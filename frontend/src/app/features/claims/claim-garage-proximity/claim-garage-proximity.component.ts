...
export class ClaimGarageProximityComponent implements OnInit, OnDestroy {
  ...
  /** Modale rendez-vous + calendrier */
  showRdvOverlay = false;
  appointmentDate: Date | null = null;
  rdvError = '';
  /** Mois affiché à l’ouverture */
  calendarStartAt = new Date();
  /** Limite haute de prise de RDV (affichage + filtre) */
  readonly maxRdvDate: Date = (() => {
    const d = new Date();
  ...
  /**
   * Dates indisponibles : passées et week-ends (ex. garage fermé).
   * Couleurs via {@link availabilityDateClass} ; sélection bloquée par {@link rdvDateFilter}.
   */
  readonly availabilityDateClass: MatCalendarCellClassFunction<Date> = (date, view) => {
    if (view !== 'month') return '';
    return this.isDateUnavailable(date) ? 'rdv-date-unavailable' : 'rdv-date-available';
  };

  readonly rdvDateFilter = (date: Date | null): boolean =>
    date != null && !this.isDateUnavailable(date);
  ...
  openRdvCalendar(): void {
    if (!this.selectedGarageId || !!this.saveSuccessMsg) return;
    this.rdvError = '';
    this.appointmentDate = null;
    this.calendarStartAt = new Date();
    this.showRdvOverlay = true;
    this.cdr.markForCheck();
  }
  ...
  private isDateUnavailable(date: Date): boolean {
    const d = ClaimGarageProximityComponent.startOfLocalDay(date);
    const today = ClaimGarageProximityComponent.startOfLocalDay(new Date());
    const maxDay = ClaimGarageProximityComponent.startOfLocalDay(this.maxRdvDate);
  ...
  closeRdvOverlay(): void {
    if (this.assigningGarage) return;
    this.showRdvOverlay = false;
    this.rdvError = '';
  }

  confirmRdvAndSaveGarage(): void {
    if (!this.appointmentDate) {
      this.rdvError = 'Veuillez choisir une date disponible (vert) dans le calendrier.';
      return;
    }
    if (this.isDateUnavailable(this.appointmentDate)) {
      this.rdvError = 'Cette date n’est pas disponible. Choisissez un jour ouvrable à venir.';
      return;
    }
    this.confirmGarageOnClaim(this.appointmentDate);
  }

  confirmGarageOnClaim(appointmentDate?: Date | null): void {
    ...
    const rdvLine =
      appointmentDate != null
        ? `\n[Rendez-vous] ${appointmentDate.toLocaleDateString('fr-FR', {
            weekday: 'long',
            day: 'numeric',
            month: 'long',
            year: 'numeric',
        ...
        : '';
    const line = `[Garage] ${g.name} (${g.city || '—'}) — ${g.distanceKm} km${coord}${rdvLine}`;
    ...
    this.claimService
      ...
      .subscribe({
        next: () => {
          ...
          this.rdvError = '';
          this.showRdvOverlay = false;
          this.saveSuccessMsg = appointmentDate
            ? `Garage « ${g.name} » et rendez-vous enregistrés sur le sinistre ${ref}.`
            : `Garage « ${g.name} » enregistré sur le sinistre ${ref}.`;
          this.saveError = '';
          this.cdr.markForCheck();
        },
        error: (err: HttpErrorResponse) => {
          ...
          const msg =
            detail && detail.length < 280
              ? detail
              : err.status === 0
                ? 'Serveur injoignable (vérifiez que le backend tourne sur le port 8082).'
                : 'Enregistrement impossible. Réessayez.';
          this.saveError = msg;
          // Visible dans la modale (le bandeau sous la carte est masqué par l’overlay)
          this.rdvError = msg;
          this.cdr.markForCheck();
        },
      });
